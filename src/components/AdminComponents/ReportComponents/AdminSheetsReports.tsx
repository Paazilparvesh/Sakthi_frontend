...
const AdminProducts: React.FC = () => {
  ...
  const flatRowsInitial = useMemo(() => flattenRows(filteredDetails), [filteredDetails]);
  ...
  const getSelectedProductIds = useCallback(() => selectedIds, [selectedIds]);
  ...
  //   // 1️⃣ Collect visible rows that match selectedIds
  //   const selectedRows = flatRows.filter(r => selected.includes(r.material_id));

  //   // 2️⃣ If total row is selected, append totalRowData
  //   if (selected.includes(TOTAL_ROW_ID)) {
  //     selectedRows.push(totalsRowData);
  //   }

  //   // 3️⃣ Log the exact export data
  //   console.log("EXPORT DATA (ALL FULL ROWS):", selectedRows);
  ...
  const response = await fetch(`${API_URL}/api/export_selected_rows/`, {
    method: "POST",
  ...
  a.href = url;
  a.download = "selected_report.xlsx";
  a.click();
};
